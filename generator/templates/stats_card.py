...
def render(stats: dict, metrics: list, theme: dict) -> str:
    ...
    for i, key in enumerate(metrics):
      ...
      <text x="0" y="6" text-anchor="middle" fill="{icon_color}" font-size="28" font-weight="bold" font-family="sans-serif" opacity="0.35" filter="url(#num-glow)">{value}</text>
      <text x="0" y="6" text-anchor="middle" fill="{theme['text_bright']}" font-size="28" font-weight="bold" font-family="sans-serif">{value}</text>
